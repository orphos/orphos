...
let new_reader () =
  ...
  let rec read_unicode_escape lexbuf limit =
    ...
    | i ->
      let ret base_char base_value = loop (acc * 16 + (Uchar.to_int (Sedlexing.lexeme_char lexbuf 0) - Char.code base_char) + base_value) (i + 1) in
      match%sedlex lexbuf with
      | '0' .. '9' -> ret '0' 0
      | 'a' .. 'f' -> ret 'a' 10
      | 'A' .. 'F' -> ret 'A' 10
      | _ -> failwith "unexpecd end of unicode escape" in
    loop 0 0 in
  ...
  let read_number lexbuf radix =
    ...
    let rec proceed m e n base_char base_value reading_exponent =
      ...
      if radix < digit_value then begin
        rollback lexbuf;
        ret m e n end
      else
        aux (m * radix + digit_value) (if reading_exponent then e + 1 else e) n reading_exponent
    and aux m e n reading_exponent =
      ...
      | '.' ->
        if reading_exponent then begin
            rollback lexbuf;
            ret m e n end
  ...
  let rec read_raw_token lexbuf =
    ...
    | Star '_', ('A' .. 'Z'), Plus ('A' .. 'Z' | '_') -> UPPER_SNAKECASE (lexeme lexbuf)
  ...
  let autoinsert_cbracket tokens =
    ...
    | WHERE :: (NL offset) :: LCBRACKET :: tl -> aux stack (LCBRACKET :: WHERE :: acc) tl
    | WHERE :: (NL offset) :: tl -> aux (offset :: stack) (LCBRACKET :: WHERE :: acc) tl
    | (NL offset) :: tl when offset < List.hd stack  -> aux (List.tl stack) (RCBRACKET :: acc) tl
  ...
  let filter_nl tokens =
    ...
    | (RCBRACKET | RPAREN | RBRACKET) :: tl -> aux (List.tl stack) (LCBRACKET :: acc) tl
  ...
  let autoinsert_semicolon tokens =
    ...
    | t1 :: (NL _ as t2) :: t3 :: tl when is_expression_end t1 && is_expression_start t3 -> aux (t3 :: t2 :: t1 :: acc) tl
    | t1 :: (NL _) :: t3 :: tl -> aux (t3 :: t1 :: acc) tl
    | NL _ :: tl -> aux acc tl
    | hd :: tl -> aux (hd :: acc) tl
    | [] -> List.rev acc in
    aux [] tokens in
  fun lexbuf -> read_raw_tokens lexbuf |> autoinsert_cbracket |> filter_nl |> autoinsert_semicolon
